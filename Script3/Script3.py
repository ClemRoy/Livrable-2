...
def write_to_csv(book_category,Links):
    os.mkdir('Script 3 Data/'+book_category)
    dirpath = 'Script 3 Data/'+ book_category+ '/' + book_category +'data.csv'    
    Book_data = []
    for link in Links:
        Book_data += book_scrapper(link)
    en_tete = ["product_page_url", "universal_product_code", "title", "price_including_tax", "price_excluding_tax", "number_available", "product_description", "category", "review_rating", "image_url"]
    with open(dirpath, 'w',  encoding="utf-8") as fichier_csv:
